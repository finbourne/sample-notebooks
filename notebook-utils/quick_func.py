...
def client_transactions(csv_file, instrument_universe):
    ...
    for index, portfolio_group, portfolio, trans_id, instr_id, ttype, units, tprice, tcurrency in transactions.itertuples():
        ...
        _client_transactions[portfolio][trans_id] = {'type': ttype,
                                                     ...
                                                     'instrument_uid': instrument_universe[instr_id]['identifiers'][
                                                         'LUID'],
                                                     'transaction_date': (yesterday_trade_open + timedelta(
                                                         hours=hour)).isoformat(),
                                                     'settlement_date': (yesterday_trade_open + timedelta(
                                                         days=hour)).isoformat(),
                                                     'units': units,
...
def fetch_client_transactions(csv_file, days_back):
    ...
    for index, transaction in transactions.iterrows():
        ...
        transaction_date = days_ago_trade_open + timedelta(days=day, hours=hour, minutes=minute, seconds=second,
